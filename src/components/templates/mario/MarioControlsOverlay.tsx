...
export const MarioControlsOverlay: React.FC<MarioControlsOverlayProps> = ({
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-20 flex flex-col justify-between">
      ...
      <header className="px-3 sm:px-6 py-2 flex items-center justify-between pointer-events-auto bg-black/40 backdrop-blur-xs text-white font-mono text-xs select-none">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 bg-[#06D6A0] hover:bg-[#05B586] text-black font-mono font-bold px-2 sm:px-3 py-1.5 rounded-lg text-[11px] border border-white transition-transform active:scale-95 shadow-md cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1 bg-[#283D52] hover:bg-[#1A2837] text-white px-2 sm:px-2.5 py-1.5 rounded-lg border border-white/40 transition-transform active:scale-95 text-xs shadow-md cursor-pointer"
      ...
      {/* Bottom Floating Game Controller ONLY for Mobile / Touch Devices without keyboard */}
      {isMobileDevice && (
        <footer className="p-3 sm:p-5 flex items-end justify-between pointer-events-auto select-none">
          {/* Left D-Pad Controls */}
          <div className="flex items-center gap-2 bg-black/50 backdrop-blur-md p-2 rounded-2xl border-2 border-white/40 shadow-2xl">
            {/* Left Arrow Button */}
            <button
              onPointerDown={onMoveLeftStart}
              onPointerUp={onMoveLeftEnd}
              onPointerLeave={onMoveLeftEnd}
              className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-[#283D52] hover:bg-[#344E69] active:bg-[#E60012] border-3 border-white text-white flex items-center justify-center transition-transform active:scale-90 shadow-lg cursor-pointer"
          ...
          <div className="flex flex-col items-center gap-1">
            <button
              ...
              className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-[#E60012] hover:bg-[#FF1A2A] active:bg-[#B3000E] border-4 border-white text-white flex flex-col items-center justify-center transition-transform active:scale-85 shadow-[0_8px_20px_rgba(230,0,18,0.6)] cursor-pointer"
